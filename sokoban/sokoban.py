...
class Sokoban:
    ...
    def automove(self):
        if self.auto == 1 and len(self.todo) > 0:
            self._move(self.todo[-1].lower())
            self.todo.pop()
        else:
            self.auto = 0
...
def main():
    ...
    # Масштабируем картинки под размер экрана
    cell_width = screen_width // skb.w
    cell_height = screen_height // skb.h
    # Берём минимальный размер, чтобы всё поместилось
    cell_size = min(cell_width, cell_height)
    
    # Создаём увеличенную версию skin
    scaled_skin = pygame.transform.scale(skin, (cell_size * 4, cell_size * 4))
    
    skb.draw(screen, scaled_skin)
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_ESCAPE:  # ESC для выхода из полноэкранного режима
                    pygame.quit()
                    sys.exit()
                elif skb.auto == 0:
                    if event.key == K_LEFT:
                ...
                if skb.auto == 0:
                    ...
                    grid_x = mousex // cell_size
                    grid_y = mousey // cell_size
                    if 0 <= grid_x < skb.w and 0 <= grid_y < skb.h:
                        skb.mouse(grid_x, grid_y)
                        skb.draw(screen, scaled_skin)
        ...
        pygame.display.set_caption(
            str(len(skb.solution)) + "/" + str(skb.push) + " - Sokoban (Полный экран, ESC выход)"
        )
